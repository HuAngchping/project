...
<html:form action="/device/SaveDiscoveryJob" method="post">
	<html:hidden property="deviceID"/>
  <logic:iterate id="nodeURI" name="DeviceJobDiscoveryForm" property="nodeURIs">
  <input type="hidden" name="nodeURIs" value="${nodeURI}"/>
  </logic:iterate>
	<table class="entityview">
	    ...
	    <tr>
	      ...
	      <td><bean:write name="device" property="model.name" /></td>
			...
			<tr>
				<th>
					<bean:message key="page.device.jobs.job.discoveryNodes.label"/>
				</th>
				<td>
          <logic:iterate id="nodeURI" name="DeviceJobDiscoveryForm" property="nodeURIs">
            <logic:notEmpty name="nodeURI"><bean:write name="nodeURI"/><br/></logic:notEmpty>
          </logic:iterate>
				</td>
			</tr>
...
<!-- // create calendar object(s) just after form tag closed
	...
	var scheduleCalendar = new calendar2(document.forms['DeviceJobDiscoveryForm'].elements['jobSchedule']);
	scheduleCalendar.year_scroll = true;
	scheduleCalendar.time_comp = true;
	
	function popupCalendar() {
	  document.forms['DeviceJobDiscoveryForm'].elements['jobScheduleOption'][1].checked = true;
