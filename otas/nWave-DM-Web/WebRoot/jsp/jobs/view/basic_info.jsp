...
<table class="entityview">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <c:if test="${provisionJob.finished}" var="jobFinished"><bean:message key="meta.job.state.Finished.label"/></c:if>
        <c:if test="${not jobFinished}">
          <bean:message key="meta.job.state.${provisionJob.state}.label"/>
        </c:if>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <th width="150"><bean:message key="page.device.jobs.job.maxRetries.label"/></th>
      <td>
        <logic:greaterThan name="provisionJob" property="maxRetries" value="0">
        <bean:write name="provisionJob" property="maxRetries"/>
        </logic:greaterThan>
      </td>
    </tr>
    <tr>
      <th width="150"><bean:message key="page.device.jobs.job.maxDuration.label"/></th>
      <td>
        <logic:greaterThan name="provisionJob" property="maxDuration" value="0">
        <bean:write name="provisionJob" property="maxDuration"/> <bean:message key="time.seconds.label"/>
        </logic:greaterThan>
      </td>
    </tr>
    <tr>
